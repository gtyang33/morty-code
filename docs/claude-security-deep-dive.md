...
- `bash` 默认通过 Linux `bwrap` 进入 OS sandbox：workspace 和 `/tmp` 可写，系统路径只读，缺少 `bwrap` 时失败关闭。
- sandbox 默认请求 network namespace 隔离；如果宿主禁止 `--unshare-net`，会保留 filesystem sandbox 并在工具结果 metadata 标记 `network_isolated: false`。
- 可通过环境变量 `MORTY_ALLOW_DANGEROUS_BASH=1` 临时绕过 bash 危险命令拦截。
- 可通过环境变量 `MORTY_DISABLE_OS_SANDBOX=1` 显式关闭 OS sandbox；可用 `MORTY_SANDBOX_NETWORK=1` 允许 sandbox 内网络。
...
- Bash 内容规则支持 Claude Code 风格的 `Bash(command)`、`Bash(prefix:*)`、`Bash(pattern*)`：
  - `Bash(uv run pytest:*)` 匹配 `uv run pytest` 及其参数。
  - `Bash(git status)` 只匹配精确命令。
  - 内容级 deny / ask 优先于工具级 allow。
...
1. **写入审批与 transcript 审计**：把权限拒绝、ask、allow 作为结构化事件写入 transcript，resume 后可追踪“为什么这个工具被拒绝”。
2. **Bash 子命令拆分**：复刻 Claude Code 对 pipe / `&&` / redirection 的分段检查，避免一个 allow prefix 包住多个实际操作。
3. **交互式 permission prompt**：在 CLI 里支持本轮允许、会话允许、项目允许、本轮拒绝、永久拒绝。
4. **严格网络 sandbox 模式**：当前会在宿主不支持 network namespace 时降级为 filesystem sandbox；后续可增加强制网络隔离失败关闭模式。
